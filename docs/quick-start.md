...
Откройте файл `.env` и заполните обязательные переменные для подключения к базе данных.

Важно: значение `DB_NAME` должно быть `yeticave`, так как это имя базы используется в `sql/schema.sql`.

SMTP-настройки можно заполнить позже, если требуется отправка email.
